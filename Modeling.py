...
class FirstOlderForward:
    ...
    def o4xFM(self, wavelet, wavalet_position, wavalet_direction='z'):
        ...
        u = np.zeros((self._nx, self._nz), dtype=np.float32)
        v = np.zeros((self._nx, self._nz), dtype=np.float32)
        r = np.zeros((self._nx, self._nz), dtype=np.float32)
        t = np.zeros((self._nx, self._nz), dtype=np.float32)
        h = np.zeros((self._nx, self._nz), dtype=np.float32)
    ...
    def o4xComputeVx(self, u, v, r, t, h):
        output = np.zeros((self._nx, self._nz), dtype=np.float32)
        
        # compute the 1st item
        kernal = np.zeros((11,11), dtype=np.float32)
...
#         kernal = np.zeros((7,7), dtype=np.float32)
...
#         return output
    ...
    def o2any_cal_r_x(self, r, cal_par):
        ...
        kernal = np.zeros((array_size, array_size), dtype=np.float32)
...
class FirstOlder_SPML(FirstOlderForward):
    ...
    def o24FM(self, wavelet, wavalet_position, wavalet_direction='z'):
        ...
        r = np.zeros((nx,nz), dtype=np.float32)
        t = np.zeros((nx,nz), dtype=np.float32)
        h = np.zeros((nx,nz), dtype=np.float32)
        
        u_ver = np.zeros((nx,nz), dtype=np.float32)
        v_ver = np.zeros((nx,nz), dtype=np.float32)
        r_ver = np.zeros((nx,nz), dtype=np.float32)
        t_ver = np.zeros((nx,nz), dtype=np.float32)
        h_ver = np.zeros((nx,nz), dtype=np.float32)
        
        u_par = np.zeros((nx,nz), dtype=np.float32)
        v_par = np.zeros((nx,nz), dtype=np.float32)
        r_par = np.zeros((nx,nz), dtype=np.float32)
        t_par = np.zeros((nx,nz), dtype=np.float32)
        h_par = np.zeros((nx,nz), dtype=np.float32)
...
#         plt.show()
    ...
    def o2xFM(self, wavelet, wavalet_position, wavalet_direction='z'):
        ...
        omega_xx = np.zeros((nx,nz), dtype=np.float32)
        omega_xz = np.zeros((nx,nz), dtype=np.float32)
        omega_zx = np.zeros((nx,nz), dtype=np.float32)
        omega_zz = np.zeros((nx,nz), dtype=np.float32)
        phi_xx = np.zeros((nx,nz), dtype=np.float32)
        phi_zz = np.zeros((nx,nz), dtype=np.float32)
        phi_zx = np.zeros((nx,nz), dtype=np.float32)
        phi_xz = np.zeros((nx,nz), dtype=np.float32)
    ...
    def DrawZWaveField(self, iterations_to_show):
        fgr, axs = plt.subplots(1,len(iterations_to_show), figsize = (18,10))
        for j, ax in enumerate(axs):
            ax.imshow(self.RangeInOne(self._vz[self._pml_len:-self._pml_len, self._pml_len:-self._pml_len, iterations_to_show[j]].T),                       cmap = plt.cm.coolwarm, vmin = -1, vmax = 1, interpolation='bilinear')
            ax.annotate("t = {0} ms".format(iterations_to_show[j]), xy=(0.05, 0.05), xycoords="axes fraction")
        plt.show()
    ...
    def DrawXWaveField(self, iterations_to_show):
        fgr, axs = plt.subplots(1,len(iterations_to_show), figsize = (18,10))
        for j, ax in enumerate(axs):
            ax.imshow(self.RangeInOne(self._vx[self._pml_len:-self._pml_len, self._pml_len:-self._pml_len, iterations_to_show[j]].T),                       cmap = plt.cm.coolwarm, vmin = -1, vmax = 1, interpolation='bilinear')
            ax.annotate("t = {0} ms".format(iterations_to_show[j]), xy=(0.05, 0.05), xycoords="axes fraction")
        plt.show()
...
class FirstOlder_NPML_PS(FirstOlder_NPML):
    ...
    def __init__(self, pmodel, vpmodel, vsmodel, tmax, xmax, zmax, nt, nx, nz, pml_len, \
        ...
        # self._pml_len = pml_len
        self._vxp = np.zeros((nx, nz, nt), dtype=np.float32)
        self._vzp = np.zeros((nx, nz, nt), dtype=np.float32)
        self._vxs = np.zeros((nx, nz, nt), dtype=np.float32)
        self._vzs = np.zeros((nx, nz, nt), dtype=np.float32)
    ...
    def o2xFM(self, wavelet, wavalet_position, wavalet_direction='z'):
        ...
        tau_p = np.zeros((nx, nz), dtype=np.float32)
        vxp = np.zeros((nx, nz), dtype=np.float32)
        vzp = np.zeros((nx, nz), dtype=np.float32)
        vxs = np.zeros((nx, nz), dtype=np.float32)
        vzs = np.zeros((nx, nz), dtype=np.float32)
    ...
    def DrawXPWaveField(self, iterations_to_show):
        fgr, axs = plt.subplots(1,len(iterations_to_show), figsize = (18,10))
        for j, ax in enumerate(axs):
            ax.imshow(self.RangeInOne(self._vxp[self._pml_len:-self._pml_len, self._pml_len:-self._pml_len, iterations_to_show[j]].T), cmap = plt.cm.coolwarm, vmin = -1, vmax = 1, interpolation='bilinear')
            ax.annotate("t = {0} ms".format(iterations_to_show[j]), xy=(0.05, 0.05), xycoords="axes fraction")
        plt.show()
    ...
    def DrawXSWaveField(self, iterations_to_show):
        fgr, axs = plt.subplots(1,len(iterations_to_show), figsize = (18,10))
        for j, ax in enumerate(axs):
            ax.imshow(self.RangeInOne(self._vxs[self._pml_len:-self._pml_len, self._pml_len:-self._pml_len, iterations_to_show[j]].T), cmap = plt.cm.coolwarm, vmin = -1, vmax = 1, interpolation='bilinear')
            ax.annotate("t = {0} ms".format(iterations_to_show[j]), xy=(0.05, 0.05), xycoords="axes fraction")
        plt.show()
    ...
    def DrawZPWaveField(self, iterations_to_show):
        fgr, axs = plt.subplots(1, len(iterations_to_show), figsize=(18, 10))
        for j, ax in enumerate(axs):
            ax.imshow(self.RangeInOne(self._vzp[self._pml_len:-self._pml_len, self._pml_len:-self._pml_len, iterations_to_show[j]].T), cmap=plt.cm.coolwarm, vmin=-1, vmax=1,
    ...
    def DrawZSWaveField(self, iterations_to_show):
        fgr, axs = plt.subplots(1, len(iterations_to_show), figsize=(18, 10))
        for j, ax in enumerate(axs):
            ax.imshow(self.RangeInOne(self._vzs[self._pml_len:-self._pml_len, self._pml_len:-self._pml_len, iterations_to_show[j]].T), cmap=plt.cm.coolwarm, vmin=-1, vmax=1,
                      interpolation='bilinear')
            ax.annotate("t = {0} ms".format(iterations_to_show[j]), xy=(0.05, 0.05), xycoords="axes fraction")
        plt.show()